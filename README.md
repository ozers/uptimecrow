...
| `WEB_ROOT` | No | `/app/web` in the image | Where the API looks for the built SPA. The default is found relative to the compiled API, so the working directory does not matter; a relative override resolves against it. An override that contains no build is ignored with a warning in favour of the default |
